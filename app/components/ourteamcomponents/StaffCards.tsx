...
export default function StaffCards({ memberData }: Props) {
  ...
  const [selectedMember, setSelectedMember] = useState<translatedMembers | null>(null);
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="staff-header text-center mb-24">
          ...
          <h2 className="text-4xl md:text-6xl text-center font-[1000] text-[#0b1236] uppercase italic tracking-tighter leading-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {memberData.map((member, i) => (
            <div
              key={i}
              onClick={() => setSelectedMember(member)} // Trigger Dialog
              className="staff-member-card group bg-white border border-slate-100 flex flex-col transition-all duration-500 hover:shadow-[0_40px_80px_-15px_rgba(11,18,54,0.1)] relative cursor-pointer"
            >
              ...
              <div className="p-8 flex-1 flex flex-col items-start text-left ">
                ...
                <div className="mt-8 pt-6 border-t border-slate-50 w-full flex justify-between items-center opacity-20">
                  <span className="font-mono text-[8px] font-bold tracking-widest uppercase italic">
                    Staff_No. {i + 101}
                  </span>
