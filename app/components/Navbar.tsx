    ...
    <>
      <header className="fixed top-0 w-full z-[100] bg-[#0b1236] border-b-2 border-[#c9a24d] shadow-2xl h-20">
        <div className="max-w-[1500px] mx-auto flex justify-between items-center h-full px-6 md:px-12">
                  ...
                  <li key={link.href} className="h-full">
                    <Link
                      ...
                      className={`h-full px-5 flex items-center text-[12px] font-black uppercase  tracking-[0.2em] transition-all relative group ${
                        isActive ? "text-[#c9a24d] bg-white/[0.03]" : "text-white/60 hover:text-white"
                      }`}
                    >
                      {link.name}
                      {isActive && <div className="absolute top-0 left-0 w-full h-[3px] bg-[#c9a24d]" />}
                    </Link>
                  </li>
                );
              })}
          ...
          >
            <span className={`h-[2px] bg-white transition-all duration-300 ${isOpen ? "w-8 rotate-45 translate-y-2" : "w-6"}`} />
            <span className={`h-[2px] bg-[#c9a24d] transition-all duration-300 ${isOpen ? "opacity-0" : "w-8"}`} />
            <span className={`h-[2px] bg-white transition-all duration-300 ${isOpen ? "w-8 -rotate-45 -translate-y-[2px]" : "w-4"}`} />
          </button>
        </div>
      </header>
      ...
      <div className={`fixed inset-0 z-[999] lg:hidden transition-all duration-500 ${isOpen ? "visible" : "invisible"}`}>
        ...
        <div className={`absolute top-0 right-0 w-[80%] max-w-[320px] h-full bg-[#0b1236] border-l-4 border-[#c9a24d] shadow-[-20px_0_50px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-out flex flex-col p-8 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <li key={link.href}>
                  <Link
                    ...
                    className={`text-xl font-black uppercase italic tracking-tight flex items-baseline gap-3 transition-colors ${
                      pathname === link.href ? "text-[#c9a24d]" : "text-white/80 hover:text-[#c9a24d]"
                    }`}
                  >
                    <span className="text-[9px] font-mono opacity-30 italic">0{i+1}</span>
                    {link.name}
                  </Link>
                </li>
              ))}
          ...
          <div className="mt-auto border-t border-white/5 pt-8 space-y-6">
            ...
            <Link 
              href="/contact" 
              className="block w-full py-4 bg-[#c9a24d] text-[#0b1236] text-center font-black uppercase italic tracking-[0.1em] text-xs shadow-lg"
            >
              {t("contact")}
            </Link>
          </div>
